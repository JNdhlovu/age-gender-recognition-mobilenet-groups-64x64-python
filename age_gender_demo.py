...
for box in boxes:
    ...
    #result = age_gender_classifier.predict(img[max(box[1], 0):box[3], max(box[0], 0):box[2]])
    result = age_gender_classifier.predict(img[y1:y2, x1:x2])
    #cv2.imwrite('face.jpg', img[max(box[1], 0):box[3], max(box[0], 0):box[2]])
    cv2.rectangle(img, (box[0], box[1]), (box[2], box[3]), (0, 0, 255))
    #cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255))
    text = "{},{}".format(result[0]['age'], result[0]['gender'])
